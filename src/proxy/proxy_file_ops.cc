...
bool Proxy::modifyFile(File &f, bool isAppend) {
    ...
    if (isAppend) {
        ...
        if (of.size < f.offset) {
            ...
            okay = false;
        } else if (f.offset % alignment != 0 || f.length % alignment != 0) {
            ...
            if (rf.data == 0) {
                ...
                if (!readFile(rf, /* is partial */ true)) {
                    rf.name = 0;
                    okay = false;
                } else {
    ...
    int numChunksPerStripe = of.numChunks / of.numStripes;
    int codingStateSize = of.codingMeta.codingStateSize / of.numStripes;
